...
The export is a report, not a table: a title block sits above the real header
row, subtotal rows sit below the items, and the currency is only stated in a
disclaimer sentence at the bottom. Amounts arrive pre-formatted for the
customer's locale, so they have to be stripped back down to numbers.

Deliberately dependency-light — csv and openpyxl only — so the API image does
not need pandas.
"""
...
def _read_xlsx(content: bytes) -> list[list[str]]:
    ...
    try:
        for sheet in book.worksheets:
            rows = [[_clean(c) for c in row] for row in sheet.iter_rows(values_only=True)]
            if any(r and r[0].lower() == HEADER_FIRST_CELL for r in rows):
                return rows
        sheet = book.worksheets[0]
        return [[_clean(c) for c in row] for row in sheet.iter_rows(values_only=True)]
    finally:
        book.close()
...
def parse_boq_file(content: bytes, filename: str) -> dict[str, Any]:
    ...
    if not filename.lower().endswith(BOQ_EXTENSIONS):
        raise ValueError(f"Unsupported file type. Accepted: {', '.join(BOQ_EXTENSIONS)}")
    if len(content) > MAX_BOQ_BYTES:
        raise ValueError(f"File too large. Maximum is {MAX_BOQ_BYTES // (1024 * 1024)} MB.")

    rows = (
        _read_xlsx(content)
        if filename.lower().endswith((".xlsx", ".xlsm"))
        else _read_csv(content)
    )
    if not rows:
    ...
    col = {name: header.index(name) for name in _EXPECTED_HEADERS}
    ...
    for row in rows[header_idx + 1:]:
        ...
        key = next(
            (
                _SUBTOTAL_LABELS[c.lower()]
                for c in row[2:amount_col]
                if c.lower() in _SUBTOTAL_LABELS
            ),
            None,
        )
        if key:
            amount = next(
                (a for a in (to_amount(c) for c in row[amount_col:]) if a is not None),
                None,
            )
        ...
        category = cell(row, col["service category"])
        service_type = cell(row, col["service type"])
        amount = to_amount(cell(row, amount_col))
