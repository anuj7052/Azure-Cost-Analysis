...
from app.services.boq_parser import parse_boq_file, to_amount
from app.services.iac_service import build_plan, to_bicep, to_terraform
...
def test_amounts_survive_locale_formatting():
    assert to_amount("? 41,668.64") == 41668.64
    assert to_amount("1 234,50") == 1234.50
...
def test_disk_lines_recover_sku_size_and_quantity(boq):
    plan = build_plan(boq)
    disks = [r for r in plan["resources"] if r["kind"] == "managed_disk"]
    assert len(disks) == 2
...
def test_bicep_output_is_scoped_and_covers_every_resource(boq):
    plan = build_plan(boq, resource_group="rg-sap")
    template = to_bicep(plan)
